...
func downloadSharadarTickers(ctx context.Context, subscription *library.Subscription, out chan<- *data.Observation, exitNotification chan<- data.RunSummary) {
	...
	for {
		...
		cursor = sharadarDownloadTickers(ctx, subscription, cursor, out)
		if cursor == "" {
			break
		}
	}
...
func sharadarDownloadTickers(ctx context.Context, subscription *library.Subscription, cursor string, out chan<- *data.Observation) string {
